...
SELECT tasks.name as task, f_name || " " || l_name as coder, teams.name as team FROM tasks
    JOIN coders ON coder_id = coders.id
    JOIN teams ON team_id = teams.id;

-- many to many
SELECT f_name || " " || l_name as coder, skills.name as skill FROM coders
    JOIN coders_skills ON coder_id = coders.id
    JOIN skills ON skill_id = skills.id;

-- one to one
SELECT f_name || " " || l_name as coder, passwords.pwd as password FROM coders
